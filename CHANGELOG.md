...
- 修复模型只在发送消息时才同步到 pi、部分 RPC 状态未返回 `thinkingLevel` 导致思考强度无法稳定调整的问题；GUI 以 pi 当前会话模型为准，用户选择模型时立即调用 `set_model`，思考等级从会话状态或记录恢复并在调整后保持。
- 会话侧栏增加“当前 / 已归档”切换，可将当前项目会话移动到 `.cagent-archive/` 并原样恢复；活动会话归档前先切换到新会话，路径越界和同名覆盖会被拒绝。
- 移除 Inspector 的“用量”标签和独立用量面板，在输入框左下角显示上下文剩余百分比；统计在初始化、模型切换、回复完成和压缩后自动刷新。
- 新增归档文件 fixture，覆盖归档、恢复、路径越界拒绝和同名覆盖拒绝。
- 已通过前端构建与 lint、Electron 语法检查、五组边界 fixture、`git diff --check` 和 `npm run package`；从桌面快捷方式直接启动后确认父进程为 `explorer.exe`，新版思考强度、归档入口、上下文剩余百分比均可见，`app.asar` 与当前源码及前端产物一致。
- 独立验收发现并修复会话初始化误写旧模型、同名 Model ID 选错 Provider、模型失败后不恢复真实状态、活动会话归档失败后 renderer 不同步，以及归档目录 junction 越界问题；模型改为联合标识的显式用户操作，归档增加预检、失败同步和真实路径校验。
